...
Output: Enhanced URLCardView.swift, updated GeneralSettingsView.swift (toggle), updated RetentionService.swift (cleanup), updated ClipboardCardView.swift (height adjustment).
</objective>
...
<task type="auto">
  ...
  **ClipboardCardView.swift update:**
  ...
  <done>
  URLCardView shows 3 states: loading (plain card + spinner), enriched (og:image banner + favicon + title), failed (plain URL fallback). Images load from disk. Animations smooth the transition. ClipboardCardView min height adjusted for URL banner cards. Build compiles cleanly.
  </done>
</task>
...
<task type="auto">
  ...
  **RetentionService.swift -- Clean up URL metadata images:**
  ...
  Also update the `clearAllHistory` method in `AppState.swift` -- wait, that file is not in this plan's files_modified list. Check if AppState.clearAllHistory also needs the same cleanup. Looking at AppState.swift line 129-133, it loops through allItems and calls deleteImage for imagePath/thumbnailPath. It needs the same favicon/preview cleanup. However, to avoid file conflicts with Plan 01 (which doesn't touch AppState), adding AppState here is safe since Plan 01 only touches ClipboardMonitor.

  Actually, let me reconsider. The files_modified already lists the files for this task. I should add AppState.swift to the cleanup too. Add to the loop in `clearAllHistory`:
  ...
  And update the deleteItem method in `ClipboardCardView.swift` similarly -- but that file is already in Task 1's scope. Actually, look at ClipboardCardView line 142-148: deleteItem already calls `deleteImage(imagePath:, thumbnailPath:)`. Add the URL metadata cleanup there too:
  ```swift
  // Clean up URL metadata cached images
  ImageStorageService.shared.deleteImage(
      imagePath: item.urlFaviconPath,
...
3. og:image banner uses 2:1 aspect ratio, scaled to fill, clipped
4. Favicon at 16pt, globe placeholder when missing
5. Title single-line truncated with ellipsis
6. Raw URL text hidden when enriched
7. Smooth animation from plain to enriched state
8. Settings "URL Previews" toggle exists, defaults to enabled
9. RetentionService cleans up favicon and og:image files
10. Individual delete and clear-all also clean up URL metadata images
</verification>
