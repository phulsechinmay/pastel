...
<task type="auto">
  ...
  <verify>
    Run: `xcodebuild build -project Pastel.xcodeproj -scheme Pastel -destination 'platform=macOS' 2>&1 | tail -30`
    Expected: Output contains "BUILD SUCCEEDED" (or at minimum, no project configuration errors -- source errors from placeholder files are acceptable at this stage).
    Run: `xcodebuild -resolvePackageDependencies -project Pastel.xcodeproj -scheme Pastel 2>&1 | tail -30`
    Expected: Package resolution succeeds for KeyboardShortcuts and LaunchAtLogin.
    Run: `grep -A1 'LSUIElement' Pastel/Resources/Info.plist`
    Expected: Output shows `<true/>` confirming no dock icon (INFR-01).
  </verify>
  <done>
